...
//         Sphere s;
//         s.center = pos;
//         s.radius = particle_radius * 12.0f; // Reduced from 4.5x - less bloating
...
// {
//     const int kernel_radius = 10; // Reduced from 15 - less blurring
//     const float far_depth = 1000.0f;
//     const float depth_threshold = 0.25f; // Tighter from 0.4 - preserve edges better
...
//     // Pass 2: Smooth with separable filter (2 passes for smooth but not overly blurred)
...
void RayTracer::updateScene(const std::vector<glm::vec3> &particle_positions, float particle_radius)
{
    ...
    for (const auto &pos : particle_positions)
    {
        Sphere s;
        s.center = pos;
        s.radius = particle_radius * 12.0f; // MUCH bigger overlap for fluid look
        s.color = glm::vec3(0.3f, 0.6f, 0.9f);
        spheres.push_back(s);
    }
